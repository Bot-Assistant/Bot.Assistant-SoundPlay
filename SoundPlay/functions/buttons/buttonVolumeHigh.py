...
class ButtonVolumeHigh(discord.ui.Button):
    def __init__(self):
        super().__init__(
            label="Volume High",
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        if not interaction.guild.voice_client.is_playing():
            await interaction.response.send_message("The bot is not playing a sound", ephemeral=True)
            return

        # Verify if the bot is not already playing a sound, if it is, make the sound volume -10%
        if interaction.guild.voice_client.is_playing():
            
            # Verify if the sound volume is already 100%, if it is, send a message to the user
            if interaction.guild.voice_client.source.volume >= 0.90:
                await interaction.response.send_message("The sound volume is already at the maximum", ephemeral=True, delete_after=5)
                return
